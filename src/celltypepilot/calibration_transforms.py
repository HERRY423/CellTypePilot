...
class IsotonicCalibrator:
    ...
    def save(self, path: str | Path):
        ...
        data = {
            ...
            if hasattr(self.model, "X_thresholds_")
            else None,
            "y_thresholds_": self.model.y_thresholds_.tolist()
            if hasattr(self.model, "y_thresholds_")
            else None,
        }
    ...
    def load(self, path: str | Path):
        ...
        self.model.X_thresholds_ = (
            np.array(data.get("X_thresholds_")) if data.get("X_thresholds_") else None
        )
        self.model.y_thresholds_ = (
            np.array(data.get("y_thresholds_")) if data.get("y_thresholds_") else None
        )
        self.model.f_ = lambda x: np.interp(x, self.model.X_thresholds_, self.model.y_thresholds_)
        self._fitted = True

    def calibration_artifact(self) -> dict:
        return {"schema_version": "celltypepilot.calibration-transform.v1", "method": "isotonic"}
...
class PlattCalibrator:
    ...
    def calibration_artifact(self) -> dict:
        return {"schema_version": "celltypepilot.calibration-transform.v1", "method": "platt"}
...
class TemperatureCalibrator:
    ...
    def fit(self, scores: np.ndarray, y_correct: np.ndarray):
        def nll(temperature):
            probs = 1 / (1 + np.exp(-scores / temperature[0]))
    ...
    def calibration_artifact(self) -> dict:
        return {"schema_version": "celltypepilot.calibration-transform.v1", "method": "temperature"}


def auto_select_calibrator(
    scores: np.ndarray, y_correct: np.ndarray, n_cv: int = 5
) -> tuple[Any, dict]:
    ...
    best_name = min(
        mean_eces, key=lambda k: mean_eces[k] if not np.isnan(mean_eces[k]) else float("inf")
    )

    best_model = models[best_name]().fit(scores, y_correct)

    report = {"selected_method": best_name, "mean_eces": mean_eces}

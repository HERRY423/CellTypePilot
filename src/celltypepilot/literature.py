...
def validate_abstained_clusters(escalation_signals: list[dict]) -> list[dict]:
    """Perform automatic literature validation for abstained or flagged clusters.

    Takes escalation signals generated by critic and runs online literature verification
    on top supporting markers, enriching signals with PubMed citations.
    """
    enriched_signals = []
    for signal in escalation_signals:
        candidate = signal.get("candidate_cell_type", "")
        markers = signal.get("supporting_markers", [])

        if candidate and candidate != "Unknown" and markers:
            try:
                lit_results = validate_annotation_with_literature(
                    cell_type=candidate,
                    positive_markers=markers[:3],
                    max_refs_per_marker=2,
                )
                signal["literature_validation"] = {
                    "assessment": lit_results.get("overall_assessment", "unknown"),
                    "total_refs": lit_results.get("total_literature_refs", 0),

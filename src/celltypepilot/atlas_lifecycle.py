...
def deprecate_cell_type(
    ...
    def _deprecate(cell_types: dict[str, Any]) -> bool:
        if cell_type in cell_types:
            ...
            return True
        return any(_deprecate(ct_info.get("subtypes", {})) for ct_info in cell_types.values())
...
def deprecate_marker_edge(
    atlas: dict[str, Any], gene: str, cell_type: str, tissue: str, reason: str
) -> None:
        ...
        if cell_type in cell_types:
            ...
            for record in records:
                if record.get("gene") == gene:
                    ...
                    found = True
            return found
        return any(_deprecate_edge(ct_info.get("subtypes", {})) for ct_info in cell_types.values())

    if not _deprecate_edge(tissue_data.get("cell_types", {})):
        logger.warning(
            f"Marker {gene} for cell type {cell_type} not found in tissue {tissue} for deprecation."
        )
...
def apply_deprecation_filter(
    markers_dict: dict[str, dict], include_deprecated: bool = False
) -> dict[str, dict]:
    ...
    for ct, info in markers_dict.items():
        ...
        filtered_info["positive_markers"] = [
            g for g in info.get("positive_markers", []) if g not in deprecated_genes
        ]
        filtered_info["negative_markers"] = [
            g for g in info.get("negative_markers", []) if g not in deprecated_genes
        ]
    ...
    def _validate(cell_types: dict[str, Any], path: str):
        for ct_name, ct_info in cell_types.items():
            ...
            if ct_info.get("deprecated") and not ct_info.get("deprecation_reason"):
                warnings.append(
                    f"Cell type {current_path} is deprecated but missing 'deprecation_reason'."
                )

            for record in ct_info.get("marker_evidence", []):
                if record.get("deprecated") and not record.get("deprecation_reason"):
                    warnings.append(
                        f"Marker edge {record.get('gene')} in {current_path} is deprecated but missing 'deprecation_reason'."
                    )

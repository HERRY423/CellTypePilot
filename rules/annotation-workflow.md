...
| "what cell types are these?" | `/annotate` (after `/inspect`) |
| "check this cluster" / "why is this a T cell?" | `/critic` |
| "what's in my h5ad?" / "inspect my data" | `/inspect` |
| "is everything installed?" / "doctor" | `/doctor` |
| "review flagged clusters" | `/critic` on each flagged cluster |

## Mandatory workflow order

1. **ALWAYS** run `/doctor` first in a new session
2. **ALWAYS** run `/inspect` before `/annotate`

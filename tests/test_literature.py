...
class TestSearchPubmed:
    ...
    def test_empty_results(self):
        ...
        mock_response.read.return_value = json.dumps({"esearchresult": {"idlist": []}}).encode(
            "utf-8"
        )
        mock_response.__enter__ = lambda s: s
        mock_response.__exit__ = MagicMock(return_value=False)

        with patch("celltypepilot.literature.urllib.request.urlopen", return_value=mock_response):
...
class TestValidateMarkerInLiterature:
    def test_no_network_returns_unknown(self):
        with (
            patch("celltypepilot.literature.search_pubmed", return_value=[]),
    ...
    def test_single_hit_partially_supported(self):
        hit = LiteratureHit(title="P", authors="A", journal="J", year=2024)
        with (
            patch("celltypepilot.literature.search_pubmed", return_value=[hit]),
            patch("celltypepilot.literature.search_biorxiv", return_value=[]),
        ):
            ev = validate_marker_in_literature("CD3E", "T cell")
    ...
    def test_multiple_hits_supported(self):
        ...
        with (
            patch("celltypepilot.literature.search_pubmed", return_value=hits),
            patch("celltypepilot.literature.search_biorxiv", return_value=[]),
        ):
            ev = validate_marker_in_literature("CD3E", "T cell")
    ...
    def test_biorxiv_fallback_supported(self):
        ...
        hit_bio = LiteratureHit(
            title="P2", authors="B", journal="bioRxiv", year=2025, source="biorxiv"
        )
        with (
            patch("celltypepilot.literature.search_pubmed", return_value=[hit_pm]),
...
class TestValidateAnnotationWithLiterature:
    def test_basic_validation(self):
        with patch("celltypepilot.literature.search_pubmed", return_value=[]):
            result = validate_annotation_with_literature("T cell", ["CD3E", "CD3D"], ["MS4A1"])
